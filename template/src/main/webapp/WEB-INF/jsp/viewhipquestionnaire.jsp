...
<div id="right_content">
 ...
 window.close();
						...
						<c:if test="${fn:length(hipquestionnaireform.hipquestionnairedetails) gt 0}">
        				 <c:forEach items="${hipquestionnaireform.hipquestionnairedetails}" var="hipquestionnairedetails" varStatus="status">
								           ...
								           <%--  <td valign="top" align="left"  width="10%"><a href="hipexamlist?hipquestionno=${hipquestionnairedetails.hipquestionno}">${hipquestionnairedetails.pname}</a></td> --%>
											...
											<a href="#" title=""><img src="resources/images/icons/icon_delete.png" alt="Delete" /></a><a href=" <c:out value="deletehipquestionnairedetails?hipquestionno=${hipquestionnairedetails.hipquestionno}"/>" onclick="return confirmation() ">Remove</a>
											</td>
	...
	<%-- <div class="extrabottom">
             ...
             <li class="page_unselect"><a href="viewhipquestionnaire_page?page=${currentpage - 1}" >Prev</a></li> 
               </c:if>
             ...
             <c:forEach var="count" begin="1" end="${noofrows}"> 
               <c:forEach begin="1" end="${noofpages}" var="i">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page_unselect"><a href="viewhipquestionnaire_page?page=${i}"><c:out value="${i}"></c:out></a></li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>          
            <c:if test="${currentpage!=noofpages}">
              <li class="page_unselect"><a href="viewhipquestionnaire_page?page=${currentpage+1}">Next</a></li> 
                 </c:if>
              <c:choose>
              <c:when test="${button=='viewall'}">
                  <li class="page"><a href="viewallhipquestionnaire" class="paging_select">ViewAll</a></li>
             </c:when>
                <c:otherwise>
                  <li class="page"><a href="viewhipquestionnaire" class="paging_select">Back</a></li>
              </c:otherwise>
              </c:choose>
							 </table>
							 </form>
